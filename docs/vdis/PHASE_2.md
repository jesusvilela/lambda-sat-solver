...
Results (EMPIRICAL, sweep completed in 858 s): frozen in
`backend/vdis/kappa_table.py`, per-κ decision totals kept as comments.
What the numbers actually support:

- **No single κ wins across families**, for either algebra — the
  precondition P4 needs to even be testable. Examples with real
  margins: C/pigeonhole strongly prefers κ=−2 (14 923 decisions vs
  38 597 at κ=−0.25, a 2.6× spread); C/random-3sat prefers κ=−0.25
  (6 342 vs 7 585 at the global −1); H/random-4sat prefers κ=−2
  (7 278 vs 9 101 at −1).
- C and H disagree about the best κ on most families (e.g. pigeonhole:
  −2 for C, −0.5 for H), so the table is keyed by (algebra, family),
  not family alone.
- 'ladder' (0 decisions everywhere — unit propagation solves it) and
  'xor-chain' (26 everywhere) tie across all κ; their recorded
  "winners" are sweep-order artifacts and flagged as such in the table.
- HYPOTHESIS, explicitly not yet supported: that the per-family table
  transfers to held-out instances of the same families. The sweep and
  the Phase 3 benchmark share generator families but Phase 3 uses
  fresh seeds and the prior session's 13-instance set — P4 is where
  this gets tested, not here.
